...
const AVOption *av_opt_next(const void *obj, const AVOption *last)
{
    ...
    if (!obj)
        return NULL;
    av_class = *(const AVClass**)obj;
    if (!last && av_class && av_class->option && av_class->option[0].name)
        return av_class->option;
    if (last && last[1].name)
        return ++last;
    return NULL;
}
...
static int write_number(void *obj, const AVOption *o, void *dst, double num, int den, int64_t intnum)
{
    ...
    switch (o->type) {
    case AV_OPT_TYPE_PIXEL_FMT:
        *(enum AVPixelFormat *)dst = (enum AVPixelFormat)llrint(num / den) * intnum;
        break;
    case AV_OPT_TYPE_SAMPLE_FMT:
        *(enum AVSampleFormat *)dst = (enum AVPixelFormat)llrint(num / den) * intnum;
        break;
    case AV_OPT_TYPE_BOOL:
    case AV_OPT_TYPE_FLAGS:
    case AV_OPT_TYPE_INT:
